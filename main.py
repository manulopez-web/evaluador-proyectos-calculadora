...
if st.button("🚀 Evaluar proyectos"):
    ...
    for proyecto in proyectos:
        ...
        tir = float(tir) if tir is not None else 0
        rbc = float(rbc) if rbc is not None else 0

        payback = calcular_payback(flujos)
        roi = calcular_roi(inversion, sum(flujos[1:]))
        indice = indice_rentabilidad(vpn, inversion)
        score = score_financiero(vpn, tir, rbc, impacto)
    ...
    # Guardar en Session State para persistencia y exportación
    ...
    if len(df) == 1:
        ...
        if fila["VPN"] > 0:
            st.success(f"🟢 El proyecto {fila['Proyecto']} presenta viabilidad financiera.")
        else:
            st.error(f"🔴 El proyecto {fila['Proyecto']} no presenta viabilidad financiera.")
    ...
    # =====================================================
        ...
        c4.metric("Costo Oportunidad", f"${costo:,.2f}")

        st.success(f"🏆 El proyecto recomendado es: {mejor['Proyecto']}")
    ...
    tasas, vpns = sensibilidad_vpn(proyecto_base["flujos"])
    sens_df = pd.DataFrame({"Tasa (%)": tasas, "VPN": vpns})
    st.session_state["sens_df"] = sens_df

    fig, ax = plt.subplots(figsize=(10,5))
    ax.plot(tasas, vpns, marker='o', linewidth=2)
    ax.axhline(0, color='red', linestyle='--', linewidth=2)
    punto = punto_critico_vpn(tasas, vpns)
    if punto:
        ax.axvline(punto, color='red', linestyle='--', linewidth=2)
    ax.set_title("Sensibilidad VPN vs Tasa")
    ...
    if punto:
        st.warning(f"⚠️ Punto crítico detectado cerca del {punto}%")
    ...
    c1.metric("VPN Promedio", f"${resultado_mc['vpn_promedio']:,.2f}")
    c2.metric("Probabilidad Éxito", f"{resultado_mc['probabilidad_exito']}%")
    c3.metric("Desviación", f"${resultado_mc['desviacion']:,.2f}")

    fig2, ax2 = plt.subplots(figsize=(10,5))
    ax2.hist(resultado_mc["vpns"], bins=40, color='skyblue', edgecolor='black')
    ax2.axvline(0, color='red', linestyle='--', linewidth=2)
    ax2.set_title("Distribución Monte Carlo del VPN")
    ax2.grid(True)
    st.pyplot(fig2)

# =========================================================
# EXPORTACIÓN (FUERA DEL BOTÓN PARA PERSISTENCIA)
# =========================================================
...
st.markdown("""
La plataforma integra análisis financiero, evaluación social, simulación probabilística 
e inteligencia artificial para apoyar la toma de decisiones estratégicas.
""")